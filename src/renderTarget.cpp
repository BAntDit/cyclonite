...
auto RenderTarget::getColorAttachment(uint8_t attachmentIndex) const -> vulkan::ImageView const&
{
    ...
    return frameBuffers_[frontBufferIndex_].getColorAttachment(hasDepth_ ? attachmentIndex + 1 : attachmentIndex);
}
...
{
    assert(hasDepth());
    return frameBuffers_[frontBufferIndex_].getColorAttachment(0);
}
