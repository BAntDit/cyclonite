...
namespace internal {
template<typename T,
         typename std::enable_if_t<std::is_enum_v<std::decay_t<T>> || std::is_integral_v<std::decay_t<T>> ||
...
template<typename T,
         typename std::enable_if_t<std::is_same_v<shared::ConstantBufferReflection, std::decay_t<T>>, int> = 0>
auto getByteSize(T&& t) -> uint64_t
{
    auto size = uint64_t{ 0 };
...
template<typename T, typename std::enable_if_t<metrix::is_iterable_v<std::decay_t<T>>, int>>
auto getByteSize(T&& t) -> uint64_t
{
    auto size = uint64_t{ 0 };
    for (auto&& v : t) {
